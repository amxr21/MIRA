...
A real-time autonomous vision and navigation pipeline for a Mars rover, running on a **Raspberry Pi + Hailo-8L NPU** with an **Arduino Mega** sensor/motor controller.
...
│
├── samples/
│   ├── input/                                # Source images (mars_01–11.jpg, photo_01–07.jpg, sample.mp4)
│   └── output/                               # Generated by tools/fuse_photos.py (git-ignored)
│
├── streaming/
...
Use `pipeline/pipeline.py` with `WINDOWS_MODE = 1` (already set by default). The `VIDEO_SOURCE` can be set to a file path or `0` for a live camera:
...
Reads all `.jpg` files from `samples/input/`, runs the full fusion pipeline (ONNX CPU), and writes annotated outputs to `samples/output/`.
