...
CONFIG = {
    ...
    "coco_path_onnx":  "yolo26n-seg.onnx",   # lives in /home/sdp2/ directly

    "unknown_iou_thresh":  0.3,
    "unknown_conf_thresh": 0.30,
    ...
    "yolo_input":  (416, 416),   # MARS HEF confirmed — DO NOT change to 640
    "coco_input":  (640, 640),   # COCO ONNX confirmed
    "depth_input": (224, 224),   # Depth HEF confirmed input size

    "record_output": "/home/sdp2/logs/fusion_output.avi",
    "record_fps":    15,
...
def _dummy_sensor() -> SensorReading:
    return SensorReading(
        ...
        tof_ok=False,
        gyro_pitch=0.0, gyro_roll=0.0, gyro_yaw=0.0,
        ldr_value=1.0,
        ultra_front=999.0, ultra_rear=999.0,
...
def _decode_hailo_dfl(bboxes: np.ndarray, scores_raw: np.ndarray,
    ...
    detections = []
    for idx in np.where(confs > 0.25)[0]:
...
class DepthEstimatorONNX:
    ...
    def __init__(self, model_path: str):
        ...
        try:
            ...
            # Test run — catches MatMul/GetElementType crash before pipeline starts
            ...
            print(f"  graph optim : DISABLED (ORT ARM mixed-precision workaround)")
...
class RoverPipeline:
        ...
        if RECORD_ENABLED:
            try:
                self.recorder = VideoRecorder(
                    ...
                    CONFIG["capture_w"], CONFIG["capture_h"])
            except Exception as e:
    ...
    def _slow_loop(self):
        ...
        while self.running:
            ...
            # Depth (may be None if model is incompatible)
            depth_map = None
            if self.depth and not getattr(self.depth,'broken',False):
                try:
